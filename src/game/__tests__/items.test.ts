...
test('rareté : nombre de bonus secondaires', () => {
  expect(makeItem('lunettes', 0, 5, rng).subs).toHaveLength(0);
  expect(makeItem('lunettes', 3, 5, rng).subs).toHaveLength(2);
  expect(makeItem('lunettes', 6, 5, rng).subs).toHaveLength(3);
});

test('fusion 3 → 1 : même objet et même rareté uniquement', () => {
  const a = makeItem('lunettes', 1, 4, rng), b = makeItem('lunettes', 1, 7, rng), c = makeItem('lunettes', 1, 5, rng);
  expect(canFuse([a, b, c])).toBe(true);
  expect(canFuse([a, b, makeItem('lunettes', 2, 5, rng)])).toBe(false);
  expect(canFuse([a, b, makeItem('griffe', 1, 5, rng)])).toBe(false);
...
test('Chromatique = rareté maximale, plus de fusion', () => {
  const x = [0, 1, 2].map(() => makeItem('griffe', 6, 1, rng));
  expect(canFuse(x)).toBe(false);
});

test('niveau et recyclage', () => {
  const it = makeItem('echarpe', 2, 3, rng);
  expect(mainValue(upgrade(it))).toBeGreaterThan(mainValue(it));
  expect(recycleValue(makeItem('echarpe', 4, 1, rng))).toBeGreaterThan(recycleValue(it));
});
